    ...
    def shape_recognition(found_shapes: List, img: cv2.typing.MatLike) -> List[Dict[str, str]]:
        ...
        for shape in found_shapes:
            ...
            if shape_points['m00'] != 0.0: 
                ...
                x, y = 0, 0  # Default coordinates if moments are zero
            
            if len(define_shape) == 3:
                cv2.drawContours(img, [shape], 0, (0, 255, 255), 5)
                cv2.putText(
                    img, 
                    f'Triangle, {shape_color}', 
                    (x, y), 
                ...
                recognized_shapes.append({'pattern': 'Triangle', 'color': shape_color, 'confidence': 'High'})
                ...
                if 0.95 <= aspect_ratio <= 1.05:
                    ...
                    recognized_shapes.append({'pattern': 'Square', 'color': shape_color, 'confidence': 'High'})
                else:
                    cv2.drawContours(img, [shape], 0, (0, 0, 0), 5)
                    cv2.putText(
                        img, 
                        f'Rectangle, {shape_color}', 
                        (x, y), 
                        cv2.FONT_HERSHEY_SIMPLEX, 
                        1, 
                        (255, 0, 255), 
                    ...
                    recognized_shapes.append({'pattern': 'Rectangle', 'color': shape_color, 'confidence': 'High'})
                
            elif len(define_shape) == 5:
                cv2.drawContours(img, [shape], 0, (0, 0, 255), 5)
                cv2.putText(
                    img, 
                    f'Pentacle, {shape_color}', 
                    (x, y), 
                    cv2.FONT_HERSHEY_SIMPLEX, 
                    1, 
                    (255, 0, 255), 
                ...
                recognized_shapes.append({'pattern': 'Pentacle', 'color': shape_color, 'confidence': 'High'})
                
            elif len(define_shape) == 6:
                cv2.drawContours(img, [shape], 0, (0, 255, 255), 5)
                cv2.putText(
                    img, 
                    f'Hexagram, {shape_color}', 
                    (x, y), 
                    cv2.FONT_HERSHEY_SIMPLEX, 
                    1, 
                    (255, 0, 255), 
                ...
                recognized_shapes.append({'pattern': 'Hexagram', 'color': shape_color, 'confidence': 'High'})
            else:
                cv2.drawContours(img, [shape], 0, (0, 0, 255), 5)
                cv2.putText(
                    img, 
                    f'Circle, {shape_color}', 
                    (x, y), 
                    cv2.FONT_HERSHEY_SIMPLEX, 
                    1, 
                    (255, 0, 255), 
                ...
                recognized_shapes.append({'pattern': 'Circle', 'color': shape_color, 'confidence': 'High'})
...
if __name__ == "__main__": 
    ...
    recognized_shapes = Detection.shape_recognition(shapes, img)
    print(recognized_shapes)
